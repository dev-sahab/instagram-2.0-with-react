...
const PostModal = ({action}) => {
    ...
    const {CLOUDINARY_URL, CLOUD_NAME} = env;
    ...
    const [postData, setPostData] = useState({
        ...
        profileImg : "https://res.cloudinary.com/dtpp4gleq/image/upload/v1676271269/Shahab_jymhye.jpg",
        post : "",
        location : "",
        photos : []
    ...
    <>
        <section id="modal">
            ...
            <div className="modal-container">
                <button className="close" onClick={() => action(false)}><img src={close} alt="" /></button>
                {
                    showLoader &&  (
                ...
                <div className={`card ${expendCard && images.length > 0 ? "expend" : ""}`}>
                    <div className="card-header">
                        <h1>Create new post</h1>
                        {expendCard ? <button onClick={handleSubmit}>Share</button> : images.length > 0 && <button onClick={() => setExpendCard(true)}>Next</button>}

                        {/* {images.length > 0 && <button onClick={() => setExpendCard(true)}>Next</button>} */}
                        
                    </div>
                    <div className={`card-body ${expendCard && images.length > 0 ? "expend" : ""}`}>
                        {!images || images.length === 0 ? 
                            (<div className="img-selector">
                                <div className="img-placeholder">
                                    <img src={photoUpload} alt="" />
                                </div>
                                <div className="selector">
                                    <h2>Drag photos and videos here</h2>
                                    <button onClick={() => fileInput.click()}>Select from computer</button>
                                    <input type="file" name="photos" ref={fileRef} onChange={handleFileChange} multiple />
                                </div>
                            </div>)
                        : 
                            (<>
                                <div className='img-preveiw'>
                                    ...
                                    >
                                    {
                                        images.map((image, index) => {
                                            const prevImg = URL.createObjectURL(image);
                                            return (
                                                <SwiperSlide><img key={index} src={prevImg} alt="" />
                                                <button className='close' onClick={() => handleDeletePhoto(image)}><img src={close} alt="" /></button></SwiperSlide>
                                            )
                                        })
                                    }
                                    </Swiper>
                                ...
                                {expendCard && (
                                    <div className="post-box">
                                        ...
                                        <div className="post-field">
                                            <textarea name="post" placeholder='Write a caption...' value={postData.post} onChange={handlePostOnchange}></textarea>
                                        </div>
                                        ...
                                        <div className="location">
                                            <div className="title">
                                                <input type="text" placeholder='Add Location' name='location' onChange={handlePostOnchange} />
                                            </div>
                                            <div className="icon">
                                                <i><CiLocationOn /></i>
                                            </div>
